...
export function chunkText(text: string, source: string): Chunk[] {
  ...
  while (start < cleaned.length) {
    ...
    // This was the final chunk (reached the end of the text) — stop here.
    // Without this, `start = end - CHUNK_OVERLAP` can recompute to the exact
    // same `start` as the previous iteration once `end` pins to
    // cleaned.length, looping forever.
